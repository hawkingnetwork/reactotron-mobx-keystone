...
export type MksPluginFilter = (ctx: ActionContext) => boolean;

export interface MksPluginOptions {
  filter?: MksPluginFilter;
 ...
 */
export function mks(opts: MksPluginOptions = {}) {
  /**
   * The mobx-keystone Reactotron plugin
   *
   * @param reactotron Reactotron instance we're attaching to
   */
  function plugin(reactotron: Reactotron) {
    if (!reactotron) {
      throw new Error('Where is Reactotron instance!!!');
    }
    ...
    const mksFilter = opts.filter ? opts.filter : () => true;

    // --- Connecting Mobx-keystone to Reactotron ---------------------------------
     ...
     */
    function trackMksNode(node: AnyModel, nodeName: string = 'default') {
      // sanity
      if (!node) {
      ...
      try {
        if (getModelDataType(node)) {
          try {
            attachReactotronToMksNode(node);
            // track this
            const nodeType = getModelDataType(node) as AnyType;
            trackedNodes[nodeName] = { node, nodeType };
            return { kind: 'ok' };
          } catch (e) {
    ...
    /**
     * Connects a mks tree node to Reactotron.
     *
     * @param node The node we want to track.
     * @param nodeName What to call this node.
     */
    function attachReactotronToMksNode(node: AnyModel, nodeName?: string) {
       ...
       */
      actionTrackingMiddleware(node, {
        filter: mksFilter,
        onFinish: (ctx: SimpleActionContext, ret) => {
          // grab the arguments
          const args = convertUnsafeArguments(ctx.args);
          const path = getRootPath(ctx.target);
          ...
          const displayPath = replace(
            /^\./,
            '',
            replace(/\//g, '.', path.path.join('/'))
          );
          let name = replace(
            /^\./,
            '',
            `${nodeName ? nodeName : ''}${displayPath}.${ctx.actionName}()`
          );
          name = replace('/', '.', name);
          ...
          return returnOrThrowActionTrackingReturn(ret);
        },
      });
    }
    ...
    function backup(command: any) {
      const trackedNode = trackedNodes[command.mksNodeName || 'default'];
    ...
    /**
     * Applies an action to the mks node which was sent from the Reactotron
        ...
        try {
          const mksAction = {
            ...
            targetPath: action.path
              ? filter(isEmpty, action.path.split('.'))
              : [],
          };

          applyAction(node, mksAction);
        } catch (err) {
    ...
    function subscribe(command: any) {
      const trackedNode = trackedNodes[command.mskNodeName || 'default'];
      const paths: string[] =
        (command && command.payload && command.payload.paths) || [];
      ...
      if (trackedNode && trackedNode.node) {
        const state =
          opts.queryMode === 'snapshot'
            ? getSnapshot(trackedNode.node)
            : trackedNode.node;
      ...
      const changes = (pipe as any)(
        ...
        map((key: string) => {
          ...
          if (is(Object, value) && !isNilOrEmpty(value)) {
            return pipe(
              keys,
              map((key: string) => `${keyMinusWildcard}.${key}`)
            )(value || {});
          }
          return [];
        }) as any,
